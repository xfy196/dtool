<script setup lang="ts">
  ...
  const form = ref({
    version: useStorage<string>(`${_function}:version`, 'v4'),
    quantity: useStorage<number>(`${_function}:quantity`, 1),
    namespace: useStorage<string>(`${_function}:namespace`, 'DNS'),
    namespaceValue: useStorage<string>(
      `${_function}:namespaceValue`,
      namespaces[0].value
    ),
    name: useStorage<string>(`${_function}:name`, '')
  });
  const rules = {
    namespaceValue: {
  ...
  const isV3V5 = computed(() => ['v3', 'v5'].includes(form.value.version));
...
<template>
  ...
  >
    ...
    <n-form-item v-show="isV3V5" label="Namespace" path="namespace">
    ...
    <n-form-item path="namespaceValue" v-show="isV3V5" label=" ">
      <n-input v-model:value="form.namespaceValue" placeholder="Namespace" />
    </n-form-item>
    <n-form-item path="name" v-show="isV3V5" label=" ">
